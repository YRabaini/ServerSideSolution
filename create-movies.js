...
if (db)
    ...
    db.serialize(function(){
        ...
        db.run("CREATE TABLE if not exists movie(id INT, year INT, title TEXT)", function(err){
            if (err)
                console.log(err.message)
                    ...
                    for (i=0; i < movies.length ; i++) {
                        if (movies[i].id && movies[i].year && movies[i].title)
            ...
            if (err)
                console.log(err.message)
                    ...
                    for (i=0; i < ratings.length ; i++) {
                        if (ratings[i].movie_id, ratings[i].rating)
